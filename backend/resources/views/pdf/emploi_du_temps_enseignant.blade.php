...
<body>
    ...
    <div class="titre-section">
        MON EMPLOI DU TEMPS — {{ $enseignant_nom }} — {{ $annee_libelle }}
    </div>
                    ...
                    @foreach($jours as $j)
                        ...
                        <td>
                            @if($cellule)
                                ...
                                <span style="font-size:9px;">{{ $cellule['classe_nom'] }}</span>
                                @if($cellule['salle'])
    ...
    <div class="resume-box">
        <strong>Total d'heures par semaine</strong><br>
        <span class="valeur">{{ $total_heures }} h</span>

        @if(count($heures_par_classe) > 0)
            <table style="width:100%; margin-top:10px;">
                <tr>
                    @foreach($heures_par_classe as $classeNom => $heures)
                        <td style="padding:2px 10px 2px 0; font-size:11px;">
                            {{ $classeNom }} : <strong>{{ $heures }} h</strong>
                        </td>
                    @endforeach
                </tr>
            </table>
